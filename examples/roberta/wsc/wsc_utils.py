...
def extended_noun_chunks(sentence):
    noun_chunks = set((np.start, np.end) for np in sentence.noun_chunks)
...
def find_span(sentence, search_text, start=0):
    ...
    for tok in sentence[start:]:
        ...
        if remainder.startswith(search_text):
            ...
            start_idx = tok.idx
            for i, next_tok in enumerate(sentence[tok.i:]):
...
def jsonl_iterator(input_fname, positive_only=False, ngram_order=3):
    ...
    with open(input_fname) as fin:
        for line in fin:
            ...
            strip_pronoun = lambda x: x.rstrip('.,"')
